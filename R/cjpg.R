#' Sessão da CJPG
#'
#' Abre uma sessão da Consulta de Julgados de Primeiro Grau.
#'
...
#' Número de páginas
#'
#' Calcula o número de páginas retornadas por uma consulta de julgados de primeiro grau.
#'
#' @param session sessão da CJPG.
#' @param parms se for \code{NULL}, admite que já está na página de resultados da CJPG.
#'
#' @return objeto de classe \code{session}.
#'
#' @examples
...
#'
#' Inclui parâmetros de pesquisa da CJPG.
#'
...
#' @param data_inicial data inicial da decisão em formado \%Y-\%m-\%d ou objeto Date.
#' @param data_final data final da decisão em formado \%Y-\%m-\%d ou objeto Date.
#' @param classes vetor com as classes que se deseja pesquisar. Obter o vetor de \code{\link{cjpg_classes}}.
#' @param assuntos vetor com os assuntos que se deseja pesquisar. Obter o vetor de \code{\link{cjpg_assuntos}}.
#' @param magistrados vetor com os magistrados que se deseja pesquisar. NOT IMPLEMENTED YET.
#' @param varas vetor com as varas que se deseja pesquisar. Obter o vetor de \code{\link{cjpg_varas}}.
#'
...
#' library(tjsp)
#' s <- cjpg_session()
#' parms <- cjsg_parms(s, livre = 'sentenca', data_inicial = '2015-01-01', data_final = '2015-05-01')
#' parms
...
#' Baixa uma página.
#'
#' Baixa uma página a partir de um resultado da CJPG. Não deve ser usado diretamente.
#'
...
#'
#' @param session sessão da CJPG, retornado pela função \code{\link{cjpg_session}}.
#' @param parms form com parâmetros de busca, retornado pela função \code{\link{cjpg_parms}}.
